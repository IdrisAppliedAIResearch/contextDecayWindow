...
R43-R48 are outside those prefixes and are written in full above.
...
| C112 | The deployed order makes the similarity tier inert, live | arm D scores 8.0/13 and arm A 8.0/13, identical on all thirteen questions, same 9/17 Q11 and 7/21 targeted, byte-identical windows at turns 117-119 | `study_011/evaluation/rubric_scores.json`; `study_011/analysis/delivery_by_arm.json` | VERIFIED - one seed, one run per arm, no variance estimate |
| C113 | K-first raises availability on the arc instrument, live | K-path episodes 1 -> 13, Q11 9/17 -> 10/17, targeted 7/21 -> 10/21 against the deployed arm | `study_011/analysis/delivery_by_arm.json` | VERIFIED - availability only; not the outcome |
...
| C119 | The N tier is not a recency window | `logical_n_key` sorts the whole store by (ever delivered, turn last delivered, source turn, id) ascending; delivered set overlaps a true window of the same size by 0.29, 36% of deliveries older than the cap of 32 turns, rotation reaches all 120 reachable episodes | `study_011/analysis/n_tier_characterization.json` | VERIFIED - replay reproduces the live ranking on 120/120 testable turns in arms A, C, D |
| C120 | Three distinct rules carry the name recency window | `StmRetrievalEngine` most-recently-delivered at cap 10; `logical_n_key` least-recently-delivered at cap 32; `episodic._recency_window` last-N at cap 32 | `study_011/analysis/n_tier_characterization.json` | VERIFIED - one probe store on which the three readings give three different answers |
| C121 | The only genuine recency window is in the component no scored live study ran | the harness imports the library's packer and renderer, not its context composition | `study_011/amendments/AMENDMENT_002_n_tier_is_not_a_recency_window.md` | VERIFIED - read from the import graph; establishes nothing about what a real window would score |
| C122 | The carried rule was a locked prefix, not a window | `retrieve()` batch-refreshes what it delivered, so the freshest-first ranking re-selects its own block; from turn 11 Study 009 Arm S and Study 007 Arm L both held source turns 1-9 plus turn t-1 for 111 consecutive turns | `study_009/analysis/n_tier_characterization.json` | VERIFIED - replay reproduces the logged ranking on 120/120 turns (Arm S), 120/120 (Arm L), 34/34 (ablation) |
| C123 | The locked prefix reaches every scored live run through Study 010 | 40 run directories scanned, 17 replay exactly, 12 lock onto the oldest episodes; Study 010's arms held source turns 1-9 across 999 logged turns at 0.11 mean overlap with a true window | `study_009/analysis/n_tier_characterization.json` | VERIFIED - Studies 001-003 do not replay and nothing is derived for them; Study 011's arms do not replay, which a test requires |
| C124 | The S-L contrast is not confounded by the tier | both arms' N blocks match turn for turn across all 120 turns; the two carried engines' decay functions agree to six places on shared inputs and share the cap of 10 | `study_009/analysis/n_tier_characterization.json` | VERIFIED - the 3.0-point gap still isolates the LTM tier and is not re-read; what changes is the baseline's description |
| C125 | Duplication precedes starvation in the deployed system's inertness | Arm D's similarity tier produced 124 candidates over 121 turns, 95 of them (76.6%) already nominated by N, and delivered a K-only episode on 1 turn in 121 | `study_011/analysis/n_tier_characterization.json` | VERIFIED - composes with IC-001 rather than replacing it; explains why relieving starvation did not pay |
| C126 | The instrument's run-to-run band is 3.0 points on the 13-point rubric | five replicates of the deployed configuration under identical corpus, settings, seed and standing runtime, back to back in one server process, scored 8.0, 8.0, 8.0, 8.0 and 11.0 | `study_011/noise_band/band_verdict.json` | VERIFIED - decision rule committed at `c07e1e27` before any replicate ran, LF digest `d412f8e0`, pinned in code so an edited rule voids the verdict |
| C127 | The band is a switch, not a spread | replicates 2-5 are byte-identical across all 121 turns; replicate 1, the only one to meet an empty server slot, diverges at turn 1 at character 79, 343 characters against 80 | `study_011/noise_band/run_manifest.json`, `runs/` | VERIFIED - the two response digests match the two Study 011 committed ones exactly; the mechanism producing the switch is not identified and is not claimed |
...
| C137 | A hard one-per-cluster floor is inert on sealed NF-006 T1 | T1 already touches 16/16 clusters; forced admissions 0; clusters 5, 8, and 15 already have one selected statement each | R48 `coverage` | VERIFIED - registered anti-vacuity stop before selector implementation or outcome measurement; no claim about a binding floor |

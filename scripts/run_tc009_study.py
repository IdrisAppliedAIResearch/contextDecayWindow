...
from analysis.tc009_study import (  # noqa: E402
    BLIND_MANIFEST_NAME,
    G0_ROOT,
    OUTCOME_ROOT,
    TC009Error,
...
def main() -> int:
    ...
    parser.add_argument("--phase", choices=("g0", "run", "worker", "blind-prefix"), required=True)
    ...
    if args.phase == "blind-prefix":
        if not args.output or not args.manifest or not args.limit:
            raise TC009Error("Blind prefix requires manifest, output and positive limit")
    ...
    if args.phase == "worker":
        if not args.output or not args.manifest:
            raise TC009Error("Worker requires manifest and output")
    ...
    if output.exists() and any(output.iterdir()):
        raise TC009Error("Registered run output is not empty")
    blind_manifest = G0_ROOT / BLIND_MANIFEST_NAME
    with tempfile.TemporaryDirectory(prefix="tc009-run-") as directory:
        ...
        if one != two:
            raise TC009Error("Fresh-process deterministic replay differed")
    ...
    (output / "run_header.json").write_text(
        json.dumps(
            {
                "schema": "tc009-run-header-v1",
                "command": ".venv\\Scripts\\python.exe scripts\\run_tc009_study.py --phase run",
                "parallel": 1,

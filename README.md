...
conditional chained-retrieval Part 2 subsequently reached Rev4, which was
authorized and then stopped at PF11: its registered context norm assumes the
unnormalized hit mean has unit norm. Score tolerance and full-ranking identity
both failed in all 12 cells, so the remaining Preflight and offline grid were
not run.

...
> gates but stops at the 35-turn reader ablation; no 120-turn run or adoption.

> **SUP-001 status:** `ABLATION_INTEGRATION_STOP - OFFLINE MECHANISM
> SUPPORTED`. Explicit accessibility changes current-only retrieval 0/64->64/64,
> preserves 32/32 unchanged facts, recovers 64/64 histories, and removes all
> stale natural selections. In the Qwen ablation C0 scores 7/9 and T1 8/9 with
> zero regressions, but T1 returns `$35.00` for locked `$35`, failing the 4/4
> unchanged bar. Explicit supersession is characterized, not promoted.
...
| SUP-001 | Explicit supersession lineage and accessibility | OFFLINE PASS; ABLATION STOP | Current-only retrieval rose 0/64 to 64/64 with 32/32 unchanged and 64/64 histories. T1 scored 8/9 in the 35-turn reader ablation but failed strict unchanged formatting (`$35.00` vs `$35`); no larger run or adoption |

"""Reachability, burden, and collision analysis for DA-073."""
...
from analysis.da073_ordered_lattice import _features, _unique

STREAMS_SHA256 = "38bbe1ffe58f4b2c27b4a8ef85512850ac17f3967bca8f83e30e5735d439caa7"
...
WORK_REFERENCE = 37
COLLISION_REFERENCE = 16


class DA073AnalysisError(RuntimeError):
    pass
...
def analyze(dataset_path: Path, directory_path: Path, streams_path: Path,
    ...
    if any(sha256_file(path) != digest for path, digest in seals):
        raise DA073AnalysisError("Sealed input differs")
    ...
    for source in _read(streams_path):
        ...
        if question_id not in target_sessions:
            continue
        ...
        session_features: dict[str, list[frozenset[str]]] = {
            str(item["session_id"]): [] for item in directories[question_id]["sessions"]
        }
        for episode in episodes:
            episode_id = str(episode["identity"])
            session_id = coordinate[episode_id]
            session_features[session_id].append(_features(
        ...
        for session_id in target_sessions[question_id]:
            size = len(groups[signatures[session_id]])
            target_group_sizes.append(size)
            ordered_collision_sessions += int(size > 1)
    if len(rows) != 465 or len(target_group_sizes) != 31:
        raise DA073AnalysisError("Outcome population differs")
    ...
    result = {
        "schema": "da073-ordered-feature-lattice-result-v1",
        "status": status,
        "questions": len(rows),
        "complete_reachability": complete,
        ...
        "target_collisions": {
            "sessions": ordered_collision_sessions,
        ...
        "by_question_type": groups_by_type,
        "simultaneously_rendered_episodes": 1,
        "peak_current_frame_chars": 2_048,
        "protected_payload_mutations": mutations,
        "calls": {"embedding": 0, "model": 0, "cache_access": 0},
        "claim_boundary": "ordered-feature reachability, burden, and collision anatomy only; no reader or delivery",
    }
    return result, rows
...
__all__ = ["DA073AnalysisError", "analyze", "run_analysis"]

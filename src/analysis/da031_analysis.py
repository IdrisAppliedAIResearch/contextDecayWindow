"""Exact cross-corpus evidence analysis for DA-031 varint pointers."""
...
class DA031AnalysisError(RuntimeError):
    pass
...
def _population_ids(path: Path) -> frozenset[str]:
    if sha256_file(path) != POPULATION_SHA256:
        raise DA031AnalysisError("LongMem population differs")
...
def analyze(locomo_path: Path, longmem_path: Path, population_path: Path,
    ...
    if sha256_file(selection_path) != SELECTION_SHA256 or sha256_file(audit_path) != AUDIT_SHA256:
        raise DA031AnalysisError("Sealed input differs")
    ...
    nf_evidence, nf_pairs, nf_groups = {}, {}, {}
    for record in adapt_split(locomo_path, HOLDOUT_IDS):
        ...
        for question in record.questions:
            ...
            nf_groups[key] = question.sample_id
    rows = []
    for key in sorted(nf_selections):
        ...
        if control and not treatment:
            raise DA031AnalysisError("NF protected control loss")
        item_key = f"{key[0]}:{key[1]}"
        rows.append({"corpus": "NF004", "key": item_key, "group": nf_groups[key],
                     "codec": selection["treatment"]["codec"], "CONTROL": control,
                     "TREATMENT": treatment, "savings": selection["treatment"]["savings"],
    ...
    for question_id in sorted(long_selections):
        ...
        control, treatment = gold <= base, gold <= treatment_set
        if control and not treatment:
            raise DA031AnalysisError("LongMem protected control loss")
    ...
    for corpus, anchor in expected.items():
        ...
        if (len(subset), complete["CONTROL"]) != anchor:
            raise DA031AnalysisError(f"{corpus} control anchor differs")
        groups = {}
    ...
    if corpora["NF004"]["contrast"]["gains"] >= 2 and corpora["LONGMEM"]["contrast"]["gains"] >= 5 and all(valid.values()):
        status = "VARINT_RELATIVE_DELIVERY_SIGNAL"
    elif any(corpora[corpus]["contrast"]["gains"] >= 2 and valid[corpus] for corpus in corpora):
        status = "PARTIAL_VARINT_RELATIVE_SIGNAL"
    else:
        status = "NO_VARINT_RELATIVE_DELIVERY_SIGNAL"
    result = {"schema": "da031-varint-result-v1", "status": status,
              "corpora": corpora, "calls": {"embedding": 0, "model": 0, "cache_access": 0},
              "claim_boundary": "spent exact availability; varint reader use and runtime unvalidated"}
    return result, rows
...
def run_analysis(locomo_path: Path, longmem_path: Path, population_path: Path,
    ...
    _write(path, rows)
    with tempfile.TemporaryDirectory(prefix="da031-result-") as directory:
    ...
    if not identical:
        raise DA031AnalysisError("Result replay differs")
    return result


__all__ = ["DA031AnalysisError", "analyze", "paired", "run_analysis"]

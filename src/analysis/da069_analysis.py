"""Reachability and burden analysis for DA-069 conjunctive signatures."""
...
STREAMS_SHA256 = "7b533e17239378a4d1926f6e2c6d0d81804ae93c1f22f7af12263df63889d1af"
REFERENCE_MEDIAN = 62


class DA069AnalysisError(RuntimeError):
    pass
...
def analyze(dataset_path: Path, streams_path: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    if sha256_file(streams_path) != STREAMS_SHA256:
        raise DA069AnalysisError("Sealed stream differs")
    ...
    if len(rows) != 465:
        raise DA069AnalysisError("Population differs")
    ...
    result = {
        "schema": "da069-maximal-conjunctive-signatures-result-v1",
        "status": status,
        "questions": len(rows),
        "complete_reachability": complete,
        ...
        "claim_boundary": "conjunctive-signature reachability and burden only; no reader or delivery",
    }
    return result, rows
...
__all__ = ["DA069AnalysisError", "analyze", "run_analysis"]

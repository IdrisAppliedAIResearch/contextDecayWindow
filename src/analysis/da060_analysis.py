"""Required-session coverage analysis for DA-060 token-pair routes."""
...
ROUTES_SHA256 = "a5e78d92cbfe32e98c53fcec12ef75f78ebff6a778330a7e0b6ee0db012c65c2"


class DA060AnalysisError(RuntimeError):
    pass
...
def analyze(dataset_path: Path, routes_path: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    if sha256_file(routes_path) != ROUTES_SHA256:
        raise DA060AnalysisError("Sealed routes differ")
    ...
    if len(rows) != 465:
        raise DA060AnalysisError("Population differs")
    ...
    if complete_rate >= .90 and p50_fraction is not None and p50_fraction <= .25:
        status = "SELECTIVE_TOKEN_PAIR_SESSION_SIGNAL"
    elif complete_rate >= .90:
        status = "BROAD_TOKEN_PAIR_SESSION_SIGNAL"
    else:
        status = "NO_TOKEN_PAIR_SESSION_SIGNAL"
    ...
    result = {"schema": "da060-token-pair-postings-result-v1", "status": status,
              ...
              "claim_boundary": "exact token-pair session address coverage only; no delivery"}
    return result, rows
...
__all__ = ["DA060AnalysisError", "analyze", "run_analysis"]

"""Required-session coverage analysis for DA-061 contiguous-bigram routes."""
...
ROUTES_SHA256 = "9fbbbb1ca232c57c9b959092de6627dfaee6fbead833f843c73ee14692c2fb4c"


class DA061AnalysisError(RuntimeError):
    pass
...
def analyze(dataset_path: Path, routes_path: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    if sha256_file(routes_path) != ROUTES_SHA256:
        raise DA061AnalysisError("Sealed routes differ")
    ...
    if len(rows) != 465:
        raise DA061AnalysisError("Population differs")
    ...
    if complete_rate >= .90 and p50_fraction is not None and p50_fraction <= .25:
        status = "SELECTIVE_CONTIGUOUS_BIGRAM_SESSION_SIGNAL"
    elif complete_rate >= .90:
        status = "BROAD_CONTIGUOUS_BIGRAM_SESSION_SIGNAL"
    else:
        status = "NO_CONTIGUOUS_BIGRAM_SESSION_SIGNAL"
    ...
    result = {
        "schema": "da061-contiguous-bigram-postings-result-v1",
        "status": status,
        "questions": len(rows),
        ...
        "claim_boundary": "exact contiguous-bigram session address coverage only; no delivery",
    }
    return result, rows
...
__all__ = ["DA061AnalysisError", "analyze", "run_analysis"]

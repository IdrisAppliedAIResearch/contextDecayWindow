"""Blind exact query-token-pair session routes for DA-060."""
...
from analysis.da059_postings import tokens


class DA060Error(RuntimeError):
    pass
...
def build_rows(dataset_path: Path, directory_path: Path) -> list[dict[str, Any]]:
    if sha256_file(dataset_path) != DATASET_SHA256 or sha256_file(directory_path) != DIRECTORY_SHA256:
        raise DA060Error("Sealed input differs")
    ...
    for question_id in sorted(directories):
        ...
        pairs = [(query_tokens[left], query_tokens[right])
                 for left in range(len(query_tokens)) for right in range(left + 1, len(query_tokens))]
        postings = {pair: [] for pair in pairs}
        observed_heads = []
        for session_order, (source_id, turns) in enumerate(
            ...
            for start in range(0, len(turns) - 1, 2):
                ...
                if first.get("role") == "user" and second.get("role") == "assistant":
                    accepted.extend((str(first.get("content", "")), str(second.get("content", ""))))
            session_tokens = set(tokens("\n".join(accepted)))
            for pair in pairs:
                if pair[0] in session_tokens and pair[1] in session_tokens:
                    postings[pair].append(session_id)
        heads = list(directories[question_id]["directory_root"]["session_heads"])
        if heads != observed_heads:
            raise DA060Error("Directory order differs")
        ...
        output.append({"question_id": question_id, "query_tokens": query_tokens,
                       "query_pairs": [[left, right] for left, right in pairs],
                       "matched_pairs": matched, "unmatched_pairs": unmatched,
                       "routed_session_ids": routed, "total_sessions": len(heads),
                       "rendered_char_delta": 0})
    if len(output) != 465:
        raise DA060Error("Population differs")
    return output
...
def run_preflight(dataset_path: Path, directory_path: Path, output_dir: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="da060-") as directory:
    ...
    matched = sum(len(row["matched_pairs"]) for row in rows)
    unmatched = sum(len(row["unmatched_pairs"]) for row in rows)
    ...
    result = {"schema": "da060-token-pair-postings-preflight-v1",
    ...
    (output_dir / "preflight.json").write_text(json.dumps(result, indent=2, sort_keys=True) + "\n",
                                                encoding="utf-8")
    if not passed:
        raise DA060Error("Blind pair-posting preflight failed")
    return result


__all__ = ["DA060Error", "build_rows", "run_preflight"]

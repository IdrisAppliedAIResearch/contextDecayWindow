"""Exact LongMem evidence analysis for DA-026 protected atomic tail."""
...
SELECTION_SHA256 = "33faa3dd3f5efbe60eae62fea64bd1ad77771768177d3a8b9362662a86c190f7"
AUDIT_SHA256 = "22225c9563bf82b12a9a9804d75e227722242148f9f7454420afc0619046d771"


class DA026AnalysisError(RuntimeError):
    pass
...
def _ids(path: Path) -> frozenset[str]:
    if sha256_file(path) != POPULATION_SHA256:
        raise DA026AnalysisError("Population differs")
...
def analyze(longmem_path: Path, population_path: Path, selection_path: Path,
    ...
    if sha256_file(selection_path) != SELECTION_SHA256 or sha256_file(audit_path) != AUDIT_SHA256:
        raise DA026AnalysisError("Sealed input differs")
    ...
    for question_id in sorted(records):
        ...
        da023 = direct | baseline | _delivered(selection["da023"]["additions"], identities)
        treatment = da023 | _delivered(selection["tail"]["actions"], identities)
        row = {"question_id": question_id, "question_type": record.question_type,
               ...
               "DA023": gold <= da023, "DA026": gold <= treatment}
        if row["DA023"] and not row["DA026"]:
            raise DA026AnalysisError("Protected control loss")
        rows.append(row)
    totals = {arm: sum(row[arm] for row in rows) for arm in ("DIRECT", "DA016", "DA023", "DA026")}
    if (totals["DIRECT"], totals["DA016"], totals["DA023"]) != (164, 188, 202):
        raise DA026AnalysisError("Control anchors differ")
    contrast = paired(rows, "DA023", "DA026")
    by_type = {}
    for kind in sorted({row["question_type"] for row in rows}):
        ...
        by_type[kind] = {"n": len(cell), "complete": {arm: sum(row[arm] for row in cell) for arm in totals},
                         "contrast": paired(cell, "DA023", "DA026")}
    wrong_rows = [row for row in rows if row["wrong_member_residual"]]
    nonnegative = all(cell["contrast"]["net"] >= 0 for cell in by_type.values())
    if contrast["losses"]:
        raise DA026AnalysisError("Protected control loss")
    if contrast["gains"] >= 5 and nonnegative:
        status = "PROTECTED_ATOMIC_TAIL_SIGNAL"
    elif contrast["gains"] >= 2 and nonnegative:
        status = "WEAK_PROTECTED_ATOMIC_TAIL_SIGNAL"
    else:
        status = "NO_PROTECTED_ATOMIC_TAIL_SIGNAL"
    result = {"schema": "da026-protected-tail-result-v1", "status": status,
              "population": len(rows), "complete": totals, "contrast_vs_da023": contrast,
              "contrast_vs_da016": paired(rows, "DA016", "DA026"), "by_question_type": by_type,
              "wrong_member_residuals": {"n": len(wrong_rows),
                                           "contrast": paired(wrong_rows, "DA023", "DA026")},
...
def run_analysis(longmem_path: Path, population_path: Path, selection_path: Path,
    ...
    _write(path, rows)
    with tempfile.TemporaryDirectory(prefix="da026-result-") as directory:
    ...
    if not identical:
        raise DA026AnalysisError("Result replay differs")
    return result


__all__ = ["DA026AnalysisError", "analyze", "paired", "run_analysis"]

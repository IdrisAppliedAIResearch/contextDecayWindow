"""TC-001B PF4: are the registered bars reachable, and are they failable?
...
catch. TC-001B registers four contrasts, so all four are probed here.
...
The A_DUAL versus A_TIERED contrast carries one extra hazard the others do not.
TC-001's verdict is public, and A_TIERED is one of its arms, so a reader could
in principle combine a directional result there with TC-001's numbers. That is
why nothing directional is emitted here for any contrast, including the ones
where it would seem harmless.
...
SCHEMA = "tc001b-preflight-pf4-reachability-v1"
...
def measure(output_dir: Path) -> dict[str, Any]:
    ...
    path = output_dir / "tc001b_preflight_pf4_reachability.json"
    path.write_text(

"""Blind shortest-codec varint allocation for DA-031."""
...
from analysis.da027_allocation import _descriptors
from analysis.da031_varint_backrefs import decode_members, encode_members, encoded_chars, prefix_for
...
class DA031Error(RuntimeError):
    pass


def _descriptors_for_control(row: Mapping[str, Any], pair_for: Any) -> list[tuple[str, tuple[int, ...]]]:
...
def _costs(role: Any, history: Sequence[str], pair: Sequence[Mapping[str, str]],
           prefix: str) -> tuple[int, tuple[int, ...]]:
    ...
    if decode_members(pair_encoded, history) != tuple(texts):
        raise DA031Error("Varint pair decode differs")
    ...
    for member, text in zip(pair, texts):
        ...
        if decode_members(encoded, history) != (text,):
            raise DA031Error("Varint member decode differs")
...
def allocate(row: Mapping[str, Any], pair_for: Any) -> dict[str, Any]:
    descriptors = _descriptors_for_control(row, pair_for)
    ...
    if decode_members(encoded, ()) != tuple(history):
        raise DA031Error("Varint immutable decode differs")
    varint_chars = role.chars - sum(map(len, history)) + encoded_chars(encoded, prefix)
    ...
    if decode_role_pairs(role) != expected:
        raise DA031Error("Immutable role sequence differs")
    ...
    if varint_chars >= control_chars:
        return {"codec": "DA028", "prefix": row["treatment"]["prefix"],
                "control_chars": control_chars, "varint_chars": varint_chars,
                "selected_chars": control_chars, "savings": 0, "final_chars": control_chars,
                "immutable_order_sha256": digest, "actions": []}

    used, seen = varint_chars, {identity for identity, _ in descriptors}
    actions = []
    ...
    return {"codec": "VARINT", "prefix": prefix, "control_chars": control_chars,
            "varint_chars": varint_chars, "selected_chars": varint_chars,
            "savings": control_chars - varint_chars, "final_chars": used,
...
def build_rows(locomo_path: Path, longmem_path: Path, population_path: Path,
    ...
    if sha256_file(da028_path) != DA028_SHA256:
        raise DA031Error("DA-028 blind artifact differs")
    ...
    if Counter(row["corpus"] for row in output) != Counter({"NF004": 1098, "LONGMEM": 465}):
        raise DA031Error("DA-031 population differs")
    return output
...
def run_preflight(locomo_path: Path, longmem_path: Path, population_path: Path,
    ...
    _write(path, rows)
    with tempfile.TemporaryDirectory(prefix="da031-") as directory:
    ...
    for corpus in ("NF004", "LONGMEM"):
        ...
        corpora[corpus] = cell
        passed = passed and all(row["treatment"]["selected_chars"] <= row["control"]["final_chars"]
                                for row in subset)
        passed = passed and codecs["VARINT"] > 0 and actions["PAIR"] > 0 and actions["TURN"] > 0 and actions["SKIP"] > 0
        passed = passed and cell["median_selected_savings"] >= 64 and cell["max_final_chars"] <= BUDGET
        if codecs["DA028"] == 0:
            passed = passed and all(row["treatment"]["varint_chars"] < row["control"]["final_chars"]
                                    for row in subset)
    result = {"schema": "da031-varint-preflight-v1", "status": "PASS" if passed else "FAIL",
    ...
    if not passed:
        raise DA031Error("DA-031 blind preflight failed")
    return result


__all__ = ["DA031Error", "allocate", "run_preflight"]

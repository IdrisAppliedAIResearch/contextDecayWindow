"""Blind immutable-order allocation with compact relative backreferences."""
...
class DA027Error(RuntimeError):
    pass


def _descriptors(row: Mapping[str, Any], pair_for: Any) -> list[tuple[str, tuple[int, ...]]]:
    output = [(str(identity), tuple(range(len(pair_for(str(identity)))))) for identity in row["direct_ids"]]
    for action in row["baseline_actions"]:
        if action["kind"] == "PAIR":
            identity = str(action["neighbor_id"])
            output.append((identity, tuple(range(len(pair_for(identity))))))
        elif action["kind"] == "TURN":
            output.append((str(action["neighbor_id"]), (int(action["member"]),)))
...
def _costs(role: Any, history: Sequence[str], pair: Sequence[Mapping[str, str]],
           prefix: str) -> tuple[int, tuple[int, ...], tuple[Any, ...], tuple[tuple[Any, ...], ...]]:
    texts = [str(member["text"]) for member in pair]
    pair_encoded = encode_members(texts, history, prefix)
    if decode_members(pair_encoded, history) != tuple(texts):
        raise DA027Error("Pair decode differs")
    full = (append_role_pair(role, pair).chars - role.chars - sum(map(len, texts))
            + encoded_chars(pair_encoded, prefix, len(history)))
    turn_costs, turn_encoded = [], []
    for member, text in zip(pair, texts):
        encoded = encode_members([text], history, prefix)
        if decode_members(encoded, history) != (text,):
            raise DA027Error("Member decode differs")
        turn_costs.append(append_role_pair(role, [member]).chars - role.chars - len(text)
                          + encoded_chars(encoded, prefix, len(history)))
        turn_encoded.append(encoded)
    return full, tuple(turn_costs), pair_encoded, tuple(turn_encoded)
...
def allocate(row: Mapping[str, Any], pair_for: Any) -> dict[str, Any]:
    ...
    if decode_members(encoded, ()) != tuple(texts):
        raise DA027Error("Immutable compact decode differs")
    immutable_compact = role.chars - sum(map(len, texts)) + encoded_chars(encoded, prefix)
    old = int(row["treatment"]["final_chars"])
    if immutable_compact > old:
        raise DA027Error("Compact immutable pack expands")
    expected = ["\n".join(f"{member['speaker']}: {member['text']}" for member in payload)
                for payload in payloads]
    if decode_role_pairs(role) != expected:
        raise DA027Error("Immutable role payload differs")
    used, history = immutable_compact, list(texts)
    ...
    return {"prefix": prefix, "immutable_old_chars": old,
            "immutable_compact_chars": immutable_compact, "immutable_savings": old - immutable_compact,
            "final_chars": used, "immutable_order_sha256":
            hashlib.sha256("\0".join(identity_order).encode()).hexdigest(), "actions": actions}
...
def build_rows(locomo_path: Path, longmem_path: Path, population_path: Path,
    ...
    if sha256_file(da023_path) != DA023_SHA256:
        raise DA027Error("DA-023 blind artifact differs")
    ...
    if Counter(row["corpus"] for row in output) != Counter({"NF004": 1098, "LONGMEM": 465}):
        raise DA027Error("DA-027 population differs")
    return output
...
def run_preflight(locomo_path: Path, longmem_path: Path, population_path: Path,
    ...
    _write(path, rows)
    with tempfile.TemporaryDirectory(prefix="da027-") as directory:
    ...
    for corpus in ("NF004", "LONGMEM"):
        ...
        savings = [row["treatment"]["immutable_savings"] for row in subset]
        cell = {"questions": len(subset), "median_immutable_savings": float(np.median(savings)),
                "p10_immutable_savings": float(np.percentile(savings, 10)),
                "p90_immutable_savings": float(np.percentile(savings, 90)),
                "actions": dict(actions), "max_final_chars": max(row["treatment"]["final_chars"] for row in subset)}
        corpora[corpus] = cell
        passed = passed and min(savings) >= 0 and cell["median_immutable_savings"] >= 64
        passed = passed and actions["PAIR"] > 0 and actions["TURN"] > 0 and actions["SKIP"] > 0
        passed = passed and cell["max_final_chars"] <= BUDGET
    result = {"schema": "da027-compact-relative-preflight-v1", "status": "PASS" if passed else "FAIL",
    ...
    if not passed:
        raise DA027Error("DA-027 blind preflight failed")
    return result


__all__ = ["DA027Error", "allocate", "run_preflight"]

"""Required-episode reachability analysis for DA-067 unigram frontiers."""
...
STREAMS_SHA256 = "3ca7d68522b0d7e1c4463fd00c26c28cad0542951ad3adf483f8c255bbc07d22"


class DA067AnalysisError(RuntimeError):
    pass
...
def analyze(dataset_path: Path, streams_path: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    if sha256_file(streams_path) != STREAMS_SHA256:
        raise DA067AnalysisError("Sealed stream differs")
    ...
    if len(rows) != 465:
        raise DA067AnalysisError("Population differs")
    ...
    status = (
        "HIERARCHICAL_UNIGRAM_FRONTIER_SIGNAL"
        if complete_rate >= .98 and mutations == 0
        else "NO_HIERARCHICAL_UNIGRAM_FRONTIER_SIGNAL"
    )
    groups = {}
    ...
    result = {
        "schema": "da067-hierarchical-unigram-frontier-result-v1",
        "status": status,
        "questions": len(rows),
        "complete_reachability": complete,
        ...
        "claim_boundary": "unigram-frontier episode reachability only; no reader, stopping, reconstruction, or delivery",
    }
    return result, rows
...
__all__ = ["DA067AnalysisError", "analyze", "run_analysis"]

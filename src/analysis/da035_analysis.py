"""Exact NF evidence analysis for DA-035 sentinel atomic composition."""
...
SELECTION_SHA256 = "97ecf4eb74b0c88914dafa6f063643249662e0b264d623a9c311a2b20e35ebd7"
AUDIT_SHA256 = "cf7764fa9e96f5621330510704d1deb976f9a623f45047010430b69b0f250cfe"


class DA035AnalysisError(RuntimeError):
    pass
...
def analyze(locomo_path: Path, selection_path: Path,
    ...
    if sha256_file(selection_path) != SELECTION_SHA256 or sha256_file(audit_path) != AUDIT_SHA256:
        raise DA035AnalysisError("Sealed input differs")
    ...
    for key in sorted(selections):
        ...
        if control and not treatment:
            raise DA035AnalysisError("Protected control loss")
    ...
    if len(rows) != 1098 or complete["CONTROL"] != 983:
        raise DA035AnalysisError("Control anchor differs")
    by_group = {}
    ...
    if contrast["gains"] >= 2 and contrast["losses"] == 0 and nonnegative:
        ...
        status = "WEAK_NF_SENTINEL_ATOMIC_SIGNAL"
    else:
        status = "NO_NF_SENTINEL_ATOMIC_SIGNAL"
    result = {"schema": "da035-nf-sentinel-atomic-result-v1", "status": status,
...
def run_analysis(locomo_path: Path, selection_path: Path,
    ...
    _write(path, rows)
    with tempfile.TemporaryDirectory(prefix="da035-result-") as directory:
    ...
    if not identical:
        raise DA035AnalysisError("Result replay differs")
    return result


__all__ = ["DA035AnalysisError", "analyze", "paired", "run_analysis"]

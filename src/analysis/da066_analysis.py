"""Required-episode reachability analysis for DA-066 dual-anchor frontiers."""
...
STREAMS_SHA256 = "821ea4b31cdeed9eea053cf81a8a4cb322f9409c92f51d0ef1c681de99c0d491"


class DA066AnalysisError(RuntimeError):
    pass
...
def analyze(dataset_path: Path, streams_path: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    if sha256_file(streams_path) != STREAMS_SHA256:
        raise DA066AnalysisError("Sealed stream differs")
    ...
    if len(rows) != 465:
        raise DA066AnalysisError("Population differs")
    ...
    status = (
        "DUAL_ANCHOR_LOCAL_FRONTIER_SIGNAL"
        if complete_rate >= .90 and mutations == 0
        else "NO_DUAL_ANCHOR_LOCAL_FRONTIER_SIGNAL"
    )
    groups = {}
    ...
    result = {
        "schema": "da066-dual-anchor-local-frontier-result-v1",
        "status": status,
        "questions": len(rows),
        "complete_reachability": complete,
        ...
        "claim_boundary": "dual-anchor episode reachability only; no reader, stopping, reconstruction, or delivery",
    }
    return result, rows
...
__all__ = ["DA066AnalysisError", "analyze", "run_analysis"]

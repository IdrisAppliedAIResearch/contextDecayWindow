...
### Phase B revision: relevance must compose along a path

The first replay of the calibrated per-link rules preserved all direct evidence and terminated, but selected at least 90% of source characters on 1,979/1,986 questions and every source unit on 1,232. Median selected/full source characters was 1.0 versus .2083 for the source-matched direct arm. These are development expansion diagnostics, not reader outcomes. Freeze that failed mechanism result as `characterization_v1`.

Before another replay, change only semantic path admission: each direct/contextual seed starts with its original query cosine; a semantic hop receives `parent_activation * max(0, min(1, link_cosine))`. Both the existing route-specific link threshold and a composed activation floor of `.48` must pass. Use the maximum activation over qualifying paths, a descending-activation queue, and stable identity tie breaks. Source attachment/member materialization does not incur semantic decay. Original direct admissions remain protected. This is a defined relevance score, not a probability; retaining `.48` as the path floor is an explicit development choice, not a transfer theorem.

The rule follows the design's requirement that weak semantic edges cannot acquire unlimited continuation merely by being individually eligible. It admits a low-query-similarity bridge through a sufficiently strong path and uses no item/depth/character selection budget. Non-increasing edge weights allow each source's best activation to finalize once; cycles cannot increase activation. Verify descending-path behavior, duplicate/equal-score cycles, and a long weak chain that must cease qualifying. Keep the same source/cue vectors and three mismatch-tail thresholds. No reader or gold labels inform this revision; record the second expansion result before deciding experimental readiness.

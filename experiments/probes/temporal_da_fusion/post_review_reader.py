...
c.OUT=BASE/'post_review_reader';c.CAP=4096;c.ARMS=['PREFIX']
OUT=c.OUT
...
def prepare():
    for p in [c.P/'POST_REVIEW_PLAN.md',Path(__file__),BASE/'gate.json',BASE/'blind.json']:c.committed(p)
    ...
    missed={r['id'] for r in c.read(BASE/'full_e_reader/result.json')['rows'] if r['score']==0}
    ...
    c.save('input_gate.json',dict(status='PASS',plan='5085934f',wrapper_sha256=c.sha(Path(__file__)),code_sha256=c.sha(c.P/'chronology.py'),
        calls=22,questions=22,native_thinking=False,output_cap=c.CAP,full_inputs_fit=True,
    ...
    print(json.dumps(dict(prepared=22,pid=proc.pid,max_tokens=max(r['tokens'] for r in schedule),output_cap=c.CAP)))
...
def run():
    ready(True)
    rows=c.read(OUT/'schedule.json');assert len(rows)==22
    for r in rows:c.capture(r,r['case']);print(r['case']+' captured',flush=True)
    ready(True);c.save('complete.json',dict(status='PASS',calls=22,hashes={r['case']:c.sha(OUT/(r['case']+'.json')) for r in rows}))

...
**Status:** OPEN - appended to as candidates are proposed, tested, or killed
...
| **F2** | **Bad cue / identity** | Q4 bundle: cosine 0.166 vs K = 0.48. Planted turn 55, probed turn 115. Rank 27 of 32. First reachable at 108,432 chars vs 32,000 budget | **CLAIMED - Family QR** |
...
| **E001** | Attention-derived term selection | **ORACLE / BOUND** | build + Q4 model in VRAM | **No** |

**Run order: E002 -> E001 (as bound) -> E003 only if the bound justifies it.**

E001 answers *how much is perfect term selection worth?* That number is only
interpretable once E002 has produced a real one to compare against.

---

### E002 - Segmented query retrieval with per-segment budget

**Type:** CANDIDATE. **Addresses:** F1 primarily, F2 possibly. **Status:** PROPOSED - **first in line.**
...
### E001 - Attention-derived term selection *(ORACLE / BOUND)*

*(Revision 3 - recategorized from candidate to oracle. Not deployable; see Deployment.)*

**Type:** ORACLE / BOUND. **Addresses:** bounds Family QR. **Status:** PROPOSED - run after E002.

**What it measures:** *if query terms could be selected perfectly, how much does
Family QR buy?* Not a shippable mechanism. The Tier 3 routing precedent: measure
the ceiling, then decide whether the family is worth building.
...
it is an oracle. **If it passes, it does not become the roadmap** - it becomes a
number that tells you what E002 and E003 are chasing.
...
the committed **0.166**. Sweep probe position - see risks.

**Interpretation, pre-committed:**
- **Bound low** (oracle barely beats E002) -> Family QR is exhausted. Do not build E003.
- **Bound high, E002 reaches it** -> ship E002. Nothing left on the table.
- **Bound high, E002 falls short** -> the gap is real and worth engineering. Build E003.
...
**Type:** CANDIDATE. **Addresses:** F1, F2. **Status:** PROPOSED - gated on E001's bound.
...
**Gated on:** E001's bound. If perfect term selection barely beats E002, the
storage cost of late interaction is not worth paying.
...
- Diversity-aware / coverage selection: MMR, submodular, facility-location. **Blocks E002 promotion.**
- Query decomposition and multi-vector retrieval for conversational memory. **Blocks E003.**
- Active / mid-generation retrieval: FLARE, IRCoT, Self-RAG.
- Items carried from `LITERATURE_LANDSCAPE.md` Section 7.

---

*Opened July 29, 2026. Revision 3, July 30, 2026 - entry types introduced;

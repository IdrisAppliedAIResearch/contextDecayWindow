...
| **F3** | **No absence detection** | E002's duplicate/zero-unique counts did not certify completeness. EC-001 then emitted 0 component abstention signals across all 500 LongMemEval-S questions, while the fixed reader answered 17/20 abstention items correctly under Codex-substituted scoring | **EXTERNALLY MEASURED — component-level absence signal absent; reader-level abstention can still succeed** |

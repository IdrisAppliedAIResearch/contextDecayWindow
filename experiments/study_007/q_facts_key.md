# Study 007 Rubric-Critical Plant Key

**Status:** LOCKED with the Study 007 pre-registration

**Carried from:** `experiments/study_006/q_facts_key.md` — **verbatim, no rows changed**. Study 007 does not touch formation, so the facts, required terms, source turns, and rubric dependencies are identical. The Study 006 lineage (and its one amendment from Study 005) is retained below unchanged.

**Re-verification for Study 007 (S7-T-001).** All 13 rows were re-checked against `experiments/study_005/script.json` (the same 121-turn script, unchanged since Study 002) for source-turn accuracy and single-span satisfiability under the locked segmenter. No discrepancy found; no row amended.

**Two matching contexts (new in Study 007).** Study 006 used this key against one object: distilled record provenance text. Study 007 uses it against two, and they are not the same object:

| Use | Object matched | Governs |
|---|---|---|
| Formation check | Distilled record text at its recorded character offsets | Bar 3 |
| Retrieval coverage check | The rendered `<retrieved_ltm>` block of the constructed prompt | Bar 1 attribution, replay gate |

The second is more permissive than the first, because the read path renders a distilled record's **whole source episode**, not its span text (see `amendments/AMENDMENT_001_delivered_information.md`). A domain can therefore be present in the LTM block via terms that were never themselves selected as spans. This is a property of the system as built, not a scoring concession, and every coverage claim in Study 007 states which object it was measured against.
...
## Bar interpretation in Study 007

The bar numbering changed between studies. What Study 006 called Bar 1 (formation) is **Study 007's Bar 3**, and it is evaluated first because it is the precondition for Bar 1 (breadth recovery). The formation criteria below are carried unchanged and now read on Study 007's Bar 3.

Study 007's **Bar 1** additionally requires four-domain coverage in the probe-turn LTM block, measured against the rendered block per the two-contexts table above and evidenced by `retrieval_budget.csv`.

## Formation criteria (Study 006 Bar 1 → Study 007 Bar 3)
